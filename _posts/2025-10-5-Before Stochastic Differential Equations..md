...
which written in the from of cylinder ($C_i$) sets equals:

s\sigma(X) = \sigma\left( \{ \bigcap_{i=1}^{n} X^{-1}(C_i): C_i = R^{i-1} \times B \times R^{n-i}, B \in \mathcal{B}_1\}\right)

σ(X)=σ({i=1⋂nX−1(Ci):Ci=Ri−1×B×Rn−i,B∈B1})## Collections of Random Variables
