...
This is possible because every linear transformation is continuous hence Borel measurable, and if $X$ is Borel measurable, then the composition is Borel measurable.

In fact, the converse is true as well, stacking two random variables will give us another random variable, to see why it is the case we need to look at the generating sets, as the generating sets are sufficient for
measurability.

So, inorder to show $X = X_1,X_2$ is mesurable, its sufficient to show that for all the measurable rectangles, the inverse image belong to $\mathcal{F}$. This is true since $X_1$ and $X_2$ are random variables.
...
$$
which written in the form of cylinder ($C_i$) sets equals:

$$\sigma(X) = \sigma\left( \left\{ \bigcap_{i=1}^{n} X^{-1}(C_i): C_i = \mathbb{R}^{i-1} \times B \times \mathbb{R}^{n-i}, B \in \mathcal{B}_1 \right\} \right)$$

Or more precisely, the σ-algebra generated by X is the σ-algebra generated by all cylinder sets of the form:

$$\sigma(X) = \sigma\left( \left\{ X^{-1}(C): C = \mathbb{R}^{i-1} \times B \times \mathbb{R}^{n-i}, B \in \mathcal{B}_1, i = 1, \ldots, n \right\} \right)$$

where $\mathcal{B}(\mathbb{R})$ denotes the Borel σ-algebra on $\mathbb{R}$.
...
Since we have had a detailed conversation regarding the sigma algebra generated by a random variable. It's natural to consider what if another random variable is measurable with respect to it.

Let $(\Omega, F, P)$ be a probability space. Let $X: \Omega \to S$ and $Y: \Omega \to T$ be random variables, where $(S,F_s)$ and $(T,F_t)$ are measurable spaces.

Then under some suitable condition, the following are equivalent:
...
The sufficient condition for this equivalence to hold is:

$(T, \mathcal{F}_T)$ separates points, i.e., all singletons $\{t\}$ in T are measurable in $\mathcal{F}_T$.

if Y is $\sigma(X)$-measurable, then Y must be constant on the level sets of X.

Here's why: For any $y \in Y(\Omega)$, the preimage $Y^{-1}(\{y\}) \in \sigma(X)$ (since singletons in T are measurable and Y is $\sigma(X)$-measurable).

Because $Y^{-1}(\{y\}) \in \sigma(X)$, there exists some $A \in \mathcal{F}_S$ such that $Y^{-1}(\{y\}) = X^{-1}(A)$.

This means: whenever $X(\omega) \in A$, we have $Y(\omega) = y$. In other words, Y takes the same value y on all points that X maps into the set A.

Therefore, we can define $f: S \to T$ by setting $f(s) = y$ for all $s \in A$, and doing this consistently for all values in $Y(\Omega)$. This gives us $Y = f(X)$.

The separation condition (singletons being measurable in T) is crucial because it allows us to "resolve" the individual values of Y through the σ-algebra structure, ensuring that Y is indeed determined by X.
Since we work with borel spaces for random variables, this holds true.
