...
In order to show uniqueness we will use the fact that any ring is a Pi-system (or in the case of measures on Borel sigma algebra, just the collection of half-boxes would suffice) and the collection of subsets where the measure is unique is a lambda system, provided that the measure is sigma-finite. Therefore the sigma algebra generated must be contained by the lambda system.
...
In the previous section we talked about uniqueness, or in general how to show any property is true for a measure. 

We need to do two things. 
* Show the property is true for some pi system that generates the sigma algebra. 
* validate that the collection of subsets of $X$ which satisfies the property is a lambda system. 

Another alternative requires defining a new algebraic structure called monotone class. It follows the similar idea: show something is true for a smaller structure which generates the larger structure.

## Definition of Monotone Classes

A collection $\mathcal{M}$ of subsets of $X$ is called a **monotone class** if it satisfies the following conditions:

1. **Upward closure**: If $(A_n)_{n=1}^{\infty}$ is an increasing sequence of sets in $\mathcal{M}$ (i.e., $A_1 \subseteq A_2 \subseteq A_3 \subseteq \cdots$), then $\bigcup_{n=1}^{\infty} A_n \in \mathcal{M}$.

2. **Downward closure**: If $(B_n)_{n=1}^{\infty}$ is a decreasing sequence of sets in $\mathcal{M}$ (i.e., $B_1 \supseteq B_2 \supseteq B_3 \supseteq \cdots$), then $\bigcap_{n=1}^{\infty} B_n \in \mathcal{M}$.

Notice that monotone classes are more restrictive than sigma algebras since they only require closure under monotone limits, not arbitrary countable unions and intersections.

## The Monotone Class Theorem

The power of monotone classes comes from the following fundamental result:

**Theorem (Monotone Class Theorem)**: Let $\mathcal{A}$ be an algebra of subsets of $X$, and let $\mathcal{M}$ be a monotone class containing $\mathcal{A}$. Then $\mathcal{M}$ contains the sigma algebra $\sigma(\mathcal{A})$ generated by $\mathcal{A}$.

In other words, the smallest monotone class containing an algebra $\mathcal{A}$ is exactly $\sigma(\mathcal{A})$.
...
3. If $f$ is measurable, then $\vert f\vert$, $\max(0,f(x))$, and $\max(0,-f(x))$ are all measurable.
